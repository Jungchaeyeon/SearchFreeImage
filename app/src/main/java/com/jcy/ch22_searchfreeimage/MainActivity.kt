...
class MainActivity : AppCompatActivity() {
    ...
    override fun onRequestPermissionsResult(
        ...
        val writeExternalStoreagePermissionGranted =
                requestCode == REQUEST_WRITE_EXTERNAL_STORAGE_PERMISSION &&
                        grantResults[0] == PackageManager.PERMISSION_GRANTED

        if(writeExternalStoreagePermissionGranted){
            fetchRandomPhotos()
        }
    }
    ...
    private fun fetchRandomPhotos(query: String? = null) = scope.launch {
        try {
            Repository.getRandomPhotos(query)?.let { photos ->
                binding.errorDescriptionView.visibility = View.GONE
            ...
            binding.errorDescriptionView.visibility = View.VISIBLE
        } finally { //항상 작업이 끝나면 실행되야할 동작
